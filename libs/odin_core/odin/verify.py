...
def _cli(argv: Optional[Sequence[str]] = None) -> int:
    ...
    src.add_argument("--envelope", help="Path to ProofEnvelope JSON or '-' for stdin")
    ...
    try:
        ...
        if args.receipt:
            ...
            data = sys.stdin.read() if args.envelope == "-" else Path(args.envelope).read_text(encoding="utf-8")
            kwargs["envelope"] = json.loads(data)
        elif args.headers:
